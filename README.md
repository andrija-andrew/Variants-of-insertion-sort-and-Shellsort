...
The project implements and analyzes standard insertion sort implementations optimized for Python, their inverted variations, and binary search optimizations. Furthermore, it features merge-like recursive variations that utilize the sorted state of sublists given by a top-down balanced binary tree function call stack. Finally, it investigates Shellsort implementations driven by distinct gap sequences, benchmarking important step sequences alongside original mathematical adaptations.
...
Except for `Shellsort2`, these implementations analyze gap sequences given in Keith McLuckie & Angus Barber's book, *Sorting Routines for Microcomputers* (Macmillan Education UK, 1986).

*   **`Shellsort0`:** The original diminishing increment sequence defined by Donald Shell (\(\lfloor n/2^k \rfloor\)).
*   **`Shellsort1`:** A frequently used sequence (\(2^k - 1\)).
*   **`Shellsort2` (Plastic Constant Adaptation):** An original, non-standard sequence driven by the geometric ratio of the **Plastic Constant** (ρ ≈ 1.3247), maintaining highly uniform gap reduction scales.
*   **`Shellsort3`:** The Papernov-Stasevich sequence based on numbers of the form \(2^k + 1\).
*   **`Shellsort4`:** Knuth's sequence based on numbers of the form \((3^k - 1) / 2\).
...
*   **Shellsort** variants are stress-tested against large lists (**100,000 to 500,000 elements**).
